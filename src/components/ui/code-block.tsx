...
import { DEFAULT_PRIMARY_HEX } from '@/src/lib/ui-theme';
...
function CopyButton({ code, floating }: { code: string; floating?: boolean }) {
  ...
  return (
    <motion.button
      ...
      className={cn(
        'relative grid size-7 place-items-center rounded-lg text-(--cb-gutter) outline-none transition-[background-color,color] duration-150 ease-out hover:bg-(--cb-hover-wash) hover:text-(--cb-plain) focus-visible:ring-2 focus-visible:ring-(--cb-accent)/60 cursor-pointer',
        copied &&
          'bg-(--cb-accent)/12 text-(--cb-accent) hover:bg-(--cb-accent)/12 hover:text-(--cb-accent)',
        floating &&
          'absolute top-2.5 right-2.5 z-10 bg-(--cb-float-bg) backdrop-blur-md',
      )}
    >
  ...
  return (
    <div
      ...
      className={cn(
        ...
        showFrame && 'rounded-2xl bg-(--cb-bg)',
        className,
      )}
      style={{ ...cssVars, ...style }}
      {...props}
    >
      {showFrame && showHeader && (
        <div
          ...
          className="flex h-10 shrink-0 items-center gap-3 bg-(--cb-header-bg) px-3.5 backdrop-blur-md"
        >
